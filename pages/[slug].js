...
function Post(props) {
  ...
  const title = `${props.title} // Nicolas Teófilo`
  const description = props.description || ''
  const url = `https://nicolasteofilo.dev/${props.slug}`
  const date = new Date(props.date).toISOString()
  const image = props.image
    ? `https://nicolasteofilo.dev${props.image}`
    : 'https://nicolasteofilo.dev/static/images/home-opt.jpg'
...
export async function getStaticProps({ params }) {
  try {
    ...
    const base = isProd ? 'https://nicolasteofilo.dev' : 'http://localhost:3000'
